...
if __name__ == '__main__':
    ...
    param = {'C': [1e3,1e1,1e-1,1e-3,1e-5]}
    print "Training LR with CV = 10"
    logreg = GridSearchCV(logreg, param, cv=5)
    logreg = logreg.fit(train_data_features, train["sentiment"])
    ...
    output.to_csv(os.path.join(os.path.dirname(__file__), 'data', 'Bag_of_Words_model_LR_CV.csv'), index=False, quoting=3 )